...
export default function GlobalChat() {
        ...
        try {
            ...
            unsubMsgRef.current = signalRService.onMessage((msg: ChatMessage) => {
                if (msg.spaceId === GLOBAL_SPACE_ID) {
                    setMessages(prev => {
                        // Check if this is a duplicate by checking if a message with same content
                        // and from same user exists in the last few seconds (potential optimistic update)
                        const now = Date.now();
                        const recentWindow = 10000; // 10 seconds window for deduplication
                        const isDuplicateFromOptimistic = prev.some(m => 
                            m.id < 0 && // optimistic message
                            m.content === msg.content &&
                            m.userId === msg.userId &&
                            (now - Math.abs(m.id)) < recentWindow
                        );
                        
                        // Also check by ID if it's already in the list
                        const alreadyExists = prev.some(m => m.id === msg.id);
                        
                        if (alreadyExists) return prev;
                        
                        if (isDuplicateFromOptimistic) {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        // Optimistic update so the message shows up in the chat immediately,
        // before the server even acknowledges it.
